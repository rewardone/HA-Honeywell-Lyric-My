...
from aiolyric import Lyric # https://github.com/timmo001/aiolyric
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def async_update_data(force_refresh_token: bool = False) -> Lyric:
        ...
        # overwrite devices_dict to enable non mac_id devices
        # for location in lyric.locations:
        #     new_devices_dict: dict = {}
        #     for device in location.devices:
        #         if device.mac_id and device.mac_id != None:
        #             _LOGGER.debug("mac_id exists: %s, no dict changes", device.mac_id)
        #             new_devices_dict[device.mac_id] = device
        #         elif device.device_id and device.device_id != None:
        #             _LOGGER.debug("device_id exists instead: %s", device.device_id)
        #             new_devices_dict[device.device_id] = device
        #         else:
        #             _LOGGER.debug("Error updating devices_dict for location.")
        #     location.devices_dict = new_devices_dict
        #     _LOGGER.debug("For location: %s, dict updated", location.location_id)
